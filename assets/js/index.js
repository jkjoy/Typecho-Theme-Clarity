...
function fileClosure(){
  ...
  tags.forEach(function(tag){
    const contentRoot = elem('.post_body');
    if (contentRoot) {
      results = contentRoot.getElementsByTagName(tag);
      Array.prototype.push.apply(headingNodes, results);
    }
  });

  headingNodes.forEach(function(node, idx){
    // Ensure headings have a stable id (for TOC and deep links)
  ...
  (function buildSidebarToc(){
    const tocNav = elem('.toc_nav');
    const tocList = tocNav ? elem('.toc_list', tocNav) : null;
    const contentRoot = elem('.post_body');

    if(!tocNav || !tocList || !contentRoot) {
      return;
    }

    const headings = headingNodes.filter(function(node){
      return (node.textContent || '').trim().length > 0;
    });

    if(!headings.length) {
      const title = tocNav.previousElementSibling;
      if(title && title.nodeName.toLowerCase() === 'h2') {
        title.remove();
      }
      tocNav.remove();
      return;
    ...
    headings.forEach(function(node){
      ...
      item.className = `toc_item toc_level_${level}`;
      const a = createEl('a');
    ...
    const links = Array.from(elems('.toc_link', tocList));
    if(!links.length) {
      const title = tocNav.previousElementSibling;
      if(title && title.nodeName.toLowerCase() === 'h2') {
        title.remove();
    ...
    function setActive(activeId) {
      links.forEach(function(a){
        const href = elemAttribute(a, 'href') || '';
        const thisId = href.startsWith('#') ? href.substring(1) : '';
        if(thisId === activeId) {
          pushClass(a, 'active');
        } else {
          deleteClass(a, 'active');
        }
      });
    }
